...
public:
  ...
  void after_rotation() override { parent->data_changed(); };
  virtual AbstractModel *GetCopy() override {

    return new BlockModel(*this);
  };
  const ModelIndex &GetModelIndex(const short &pos_x,
                                  const short &pos_y) override;
  void SetModelIndex(const ModelIndex &smth_data, const short &pos_x,
                     const short &pos_y) override;
  ;
  ...
  void Rotate() override { this->RotateRight(); }
  void CancelRotate() override { this->RotateLeft(); }
