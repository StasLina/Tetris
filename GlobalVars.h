...
public:
    ...
    friend bool operator<=(GlobalVars& index1, const GlobalVars& index2) {
        if (index1.x_size <= index2.x_size && index1.y_size <= index2.y_size) {
            return true;
        }
        else {
            return false;
    ...
    public:
        ...
        char data = ' ';
        //pos position;
        friend std::ostream& operator<<(std::ostream& os, const ModelIndex& index) {
    ...
    class AbstractModel {
    public:
        virtual const ModelIndex& GetModelIndex(short const& pox_x, const short& pos_y) { return ModelIndex(); };
        virtual void SetModelIndex(const ModelIndex& smth_index, const short& pox_x, const short& pos_y) {};
        virtual unsigned short GetXSize() { return 0; }
        virtual unsigned short GetYSize() { return 0; }
        virtual void SetModelSize(unsigned short x, unsigned short y) { return; };
    ...
    public:
        const ModelIndex& GetModelIndex(short const& pos_x, const short& pos_y) override {
            auto row = data.begin();
            short n = 0;
            while (n != pos_y) {
                ++row;
        ...
        void SetModelIndex(const ModelIndex& smth_index, const short& pos_x, const short& pos_y) override {
            auto row = data.begin();
            short n = 0;
            while (n != pos_y) {
                ++row;
    ...
    public:
        ...
        void DeleteRow(int smth_index_row) {
            std::list<std::vector<ModelIndex>>::iterator it= this->data.begin();
            for (int i = 0; i != smth_index_row; ++i, ++it);
            this->data.erase(it);
        ...
        bool isMax() {
            if(this->cur_max >= this->GetYSize()-1)return true;
            return false;
        }
    };
#endif // !GLOBAL_VARS_H
